...
object Main extends IOApp.Simple:
  ...
  def run: IO[Unit] =
    for
      value <- calculateDay1("src/main/resources/day1_input.txt")
      _ <- IO.println(value)
    yield ()
  ...
  def move(value: Ref[IO, Int], total: Ref[IO, Int], instruction: Instruction): IO[Unit] = {
    ...
    for
      ...
      total <- total.get
      _ <- IO.println(s"Instruction: $instruction, Current value: $newValue, Total: $total")
    yield ()
  }
  ...
  private def readDay1File(path: String): IO[List[(Char, Int)]] =
    Files[IO]
      .readAll(Path(path))
